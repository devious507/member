...
$sql="SELECT c.cardnumber,c.void,m.keycard_number,m.namelast||', '||m.namefirst,m.phone,m.spouse_last||', '||m.spouse_first,m.spouse_phone,c.note FROM membershipcards AS c LEFT OUTER JOIN members AS m ON c.memberid=m.memberid WHERE c.expirationyear={$_GET['year']} ORDER BY m.namelast,m.namefirst";
$res = simpleQuery($sql,true,$db);
...
foreach($data as $row) {
	...
	foreach($row as $k=>$v) {
		if($void == 1) {
			...
			if($k == 'cardNumber') {
				$body.="<td style=\"background-color: {$background};font-size: small\"><a href=\"utility_lookupByCardNumber.php?search=Search&expirationYear={$_GET['year']}&cardNumber={$v}\">{$v}</td>";
			} else {
